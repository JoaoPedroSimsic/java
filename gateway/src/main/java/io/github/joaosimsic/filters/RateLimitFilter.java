...
public class RateLimitFilter implements GlobalFilter, Ordered {
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    RateDetails config =
        isAuthenticated
            ? props.rateLimit().authenticated()
            : props.rateLimit().unauthenticated();
    ...
    return redisTemplate
        ...
        .flatMap(
            currentCount -> {
              ...
              int remaining = Math.max(0, config.burstCapacity() - (int) count - 1);
        ...
        .onErrorResume(
            e -> {
              ...
              return Mono.just(
                  new RateLimitResult(true, config.burstCapacity(), WINDOW_SIZE.toSeconds()));
            });
  }
  ...
  private void addRateLimitHeaders(
    ...
    exchange
        .getResponse()
        .getHeaders()
        .add("X-RateLimit-Limit", String.valueOf(config.burstCapacity()));
    exchange
        .getResponse()
        .getHeaders()
